...
public class MediaSessionCompat {
    ...
    public static final class QueueItem implements Parcelable {
        ...
        public String toString() {
            StringBuilder sbR = a.r("MediaSession.QueueItem {Description=");
            sbR.append(this.mDescription);
            sbR.append(", Id=");
            sbR.append(this.mId);
            sbR.append(" }");
            return sbR.toString();
    ...
    public static PlaybackStateCompat getStateWithUpdatedPosition(PlaybackStateCompat playbackStateCompat, MediaMetadataCompat mediaMetadataCompat) {
        ...
        return new PlaybackStateCompat.Builder(playbackStateCompat).setState(playbackStateCompat.getState(), (j2 < 0 || position <= j2) ? position < 0 ? 0L : position : j2, playbackStateCompat.getPlaybackSpeed(), jElapsedRealtime).build();
    }
    ...
    public MediaSessionCompat(@NonNull Context context, @NonNull String str, @Nullable ComponentName componentName, @Nullable PendingIntent pendingIntent, @Nullable Bundle bundle, @Nullable VersionedParcelable versionedParcelable) {
        ...
        if (context != null) {
            if (!TextUtils.isEmpty(str)) {
                ...
                if (i2 >= 29) {
                    this.mImpl = new MediaSessionImplApi29(mediaSessionCreateFwkMediaSession, versionedParcelable, bundle);
                } else if (i2 >= 28) {
                    this.mImpl = new MediaSessionImplApi28(mediaSessionCreateFwkMediaSession, versionedParcelable, bundle);
                } else {
                    this.mImpl = new MediaSessionImplApi21(mediaSessionCreateFwkMediaSession, versionedParcelable, bundle);
                }
