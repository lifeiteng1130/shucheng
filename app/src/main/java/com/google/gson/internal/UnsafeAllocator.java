...
public abstract class UnsafeAllocator {
    public static void assertInstantiable(Class<?> cls) {
        ...
        if (Modifier.isInterface(modifiers)) {
            StringBuilder sbR = a.r("Interface can't be instantiated! Interface name: ");
            sbR.append(cls.getName());
            throw new UnsupportedOperationException(sbR.toString());
        }
        if (Modifier.isAbstract(modifiers)) {
            StringBuilder sbR2 = a.r("Abstract class can't be instantiated! Class name: ");
            sbR2.append(cls.getName());
            throw new UnsupportedOperationException(sbR2.toString());
        }
    }
