...
public class Menubar extends JMenuBar implements ActionListener {
    ...
    public Menubar(SwingGui swingGui) {
        ...
        while (i2 < 4) {
            if (strArr3[i2].length() == 0) {
                ...
                strArr = strArr6;
                strArr2 = strArr3;
                JMenuItem jMenuItem = new JMenuItem(strArr3[i2], cArr[i2]);
                jMenuItem.setActionCommand(strArr4[i2]);
        ...
        for (int i3 = 0; i3 < 5; i3++) {
            ...
            if (iArr2[i3] != 0) {
                jMenuItem2.setAccelerator(KeyStroke.getKeyStroke(iArr2[i3], 2));
            }
        }
        for (int i4 = 0; i4 < 3; i4++) {
            JMenuItem jMenuItem3 = new JMenuItem(strArr7[i4], cArr4[i4]);
            jMenuItem3.addActionListener(this);
            jMenu3.add(jMenuItem3);
        }
        int i5 = 0;
        while (i5 < 5) {
            JMenuItem jMenuItem4 = new JMenuItem(strArr8[i5], cArr3[i5]);
            jMenuItem4.addActionListener(this);
            if (iArr3[i5] != 0) {
                jMenuItem4.setAccelerator(KeyStroke.getKeyStroke(iArr3[i5], 0));
            }
            if (i5 != 0) {
    ...
    public void updateEnabled(boolean z) {
        for (int i2 = 0; i2 != this.interruptOnlyItems.size(); i2++) {
            this.interruptOnlyItems.get(i2).setEnabled(z);
        }
        for (int i3 = 0; i3 != this.runOnlyItems.size(); i3++) {
            this.runOnlyItems.get(i3).setEnabled(!z);
        }
    }
}
