...
public class SubStringAfterLast implements Function {
    ...
    public XValue call(Scope scope, List<XValue> list) {
        ...
        if (!f.e(strAsString)) {
            strAsString = (f.e(strAsString2) || (iLastIndexOf = strAsString.lastIndexOf(strAsString2)) == -1 || iLastIndexOf == strAsString.length() - strAsString2.length()) ? "" : strAsString.substring(strAsString2.length() + iLastIndexOf);
        }
