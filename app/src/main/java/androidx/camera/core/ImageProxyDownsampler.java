...
public final class ImageProxyDownsampler {
    ...
    public static ForwardingImageProxy downsample(ImageProxy imageProxy, int i2, int i3, DownsamplingMethod downsamplingMethod) {
        ...
        if (imageProxy.getWidth() < i2 || imageProxy.getHeight() < i3) {
            StringBuilder sbR = a.r("Downsampled dimension ");
            sbR.append(new Size(i2, i3));
            sbR.append(" is not <= original dimension ");
            sbR.append(new Size(imageProxy.getWidth(), imageProxy.getHeight()));
            sbR.append(".");
        ...
        synchronized (byteBuffer) {
            ...
            while (i10 < i7) {
                ...
                for (int i14 = 0; i14 < i6; i14++) {
                    bArr[i13 + i14] = (byte) ((((((bArr2[iArr[i14]] & ExifInterface.MARKER) + (bArr2[iArr[i14] + i3] & ExifInterface.MARKER)) + (bArr3[iArr[i14]] & ExifInterface.MARKER)) + (bArr3[iArr[i14] + i3] & ExifInterface.MARKER)) / 4) & 255);
                }
                i10++;
                i8 = 0;
            }
