    ...
    private void writeExifSegment(@NonNull ByteOrderedDataOutputStream byteOrderedDataOutputStream) throws IOException {
        ...
        for (int i5 = 0; i5 < ExifData.EXIF_TAGS.length; i5++) {
            if (!this.mExifData.getAttributes(i5).isEmpty()) {
                iArr[i5] = size2;
                size2 = (this.mExifData.getAttributes(i5).size() * 12) + 2 + 4 + iArr2[i5] + size2;
            }
        }
        int i6 = size2 + 8;
        if (!this.mExifData.getAttributes(1).isEmpty()) {
        ...
        for (int i7 = 0; i7 < ExifData.EXIF_TAGS.length; i7++) {
            if (!this.mExifData.getAttributes(i7).isEmpty()) {
                ...
                int size3 = (this.mExifData.getAttributes(i7).size() * 12) + iArr[i7] + 2 + 4;
                for (Map.Entry<String, ExifAttribute> entry : this.mExifData.getAttributes(i7).entrySet()) {
                    ExifTag exifTag2 = ExifData.Builder.sExifTagMapsForWriting.get(i7).get(entry.getKey());
                    StringBuilder sbR = a.r("Tag not supported: ");
                    sbR.append(entry.getKey());
                    sbR.append(". Tag needs to be ported from ExifInterface to ExifData.");
                    int i8 = ((ExifTag) Preconditions.checkNotNull(exifTag2, sbR.toString())).number;
