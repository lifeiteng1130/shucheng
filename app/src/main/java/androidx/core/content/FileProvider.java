...
public class FileProvider extends ContentProvider {
    ...
    private static final File DEVICE_ROOT = new File(w.DEFAULT_PATH_SEPARATOR);
    ...
    public static class SimplePathStrategy implements PathStrategy {
        ...
        public File getFileForUri(Uri uri) {
            ...
            if (file == null) {
                throw new IllegalArgumentException(a.g("Unable to find configured root for ", uri));
            }
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                return new Uri.Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode(entry.getKey()) + Attributes.InternalPrefix + Uri.encode(path2.endsWith(w.DEFAULT_PATH_SEPARATOR) ? canonicalPath.substring(path2.length()) : canonicalPath.substring(path2.length() + 1), w.DEFAULT_PATH_SEPARATOR)).build();
            } catch (IOException unused) {
