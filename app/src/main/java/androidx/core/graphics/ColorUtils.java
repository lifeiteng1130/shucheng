...
public final class ColorUtils {
    ...
    public static double calculateContrast(@ColorInt int i2, @ColorInt int i3) {
        if (Color.alpha(i3) != 255) {
            StringBuilder sbR = a.r("background can not be translucent: #");
            sbR.append(Integer.toHexString(i3));
    ...
    public static Color compositeColors(@NonNull Color color, @NonNull Color color2) {
        ...
        StringBuilder sbR = a.r("Color models must match (");
        sbR.append(color.getModel());
        sbR.append(" vs. ");
        sbR.append(color2.getModel());
        sbR.append(")");
        throw new IllegalArgumentException(sbR.toString());
    }
}
