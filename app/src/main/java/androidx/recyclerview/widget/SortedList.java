...
public class SortedList<T> {
    ...
    public static abstract class Callback<T2> implements Comparator<T2>, ListUpdateCallback, j$.util.Comparator {
    ...
    private void addToData(int i2, T t) {
        int i3 = this.mSize;
        if (i2 > i3) {
            StringBuilder sbS = a.s("cannot add item to ", i2, " because size is ");
            sbS.append(this.mSize);
    ...
    public T get(int i2) {
        ...
        StringBuilder sbS = a.s("Asked to get item at ", i2, " but size is ");
        sbS.append(this.mSize);
        throw new IndexOutOfBoundsException(sbS.toString());
    }
