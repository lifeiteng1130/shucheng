...
public final class FlingAnimation extends DynamicAnimation<FlingAnimation> {
    ...
    public static final class DragForce implements Force {
        ...
        public DynamicAnimation.MassState updateValueAndVelocity(float f2, float f3, long j2) {
            float f4 = j2;
            this.mMassState.mVelocity = (float) (Math.exp((f4 / 1000.0f) * this.mFriction) * ((double) f3));
            DynamicAnimation.MassState massState = this.mMassState;
            float f5 = this.mFriction;
            massState.mValue = (float) ((Math.exp((f5 * f4) / 1000.0f) * ((double) (f3 / f5))) + ((double) (f2 - (f3 / f5))));
